...
async function atualizarStatusEntrega(wamid, novoStatus) {
  try {
    const { error } = await supabase.from("mensagens").update({ status_entrega: novoStatus }).eq("wamid", wamid);
...
app.post("/webhook", async (req, res) => {
  ...
  if (body.object === "whatsapp_business_account") {
    ...
    if (statusEvent) {
      ...
      atualizarStatusEntrega(wamid, status).catch(() => {});
      return;
    }
  ...
  try {
    ...
    let msgsHtml = mensagens.map(m => {
      ...
      if (tipo !== 'client' && m.wamid) {
        ...
        statusIcone = ' <span title="' + s.titulo + '" style="color:' + s.cor + '">' + s.icone + '</span>';
      }
