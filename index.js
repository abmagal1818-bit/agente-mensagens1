...
async function processarComandoConsultor(from, text) {
  if (!ehConsultor(from)) {
    console.log(`[Consultor] NÃO é consultor: from=${from}`);
    return false;
  }
  const t = text.trim().toUpperCase();
  console.log(`[Consultor] É consultor. Texto: "${t}", length=${t.length}`);
  ...
  console.log(`[Consultor] ehComando=${ehComando}`);
...
app.post("/webhook", async (req, res) => {
  ...
  if (body.object === "whatsapp_business_account") {
    ...
    const _isConsultor = from && String(from).replace(/\D/g,"").slice(-10) === String(NUMERO_AUGUSTO).replace(/\D/g,"").slice(-10);
    console.log(`[DEBUG] from=${from} NUMERO_AUGUSTO=${NUMERO_AUGUSTO} isConsultor=${_isConsultor} text="${msg.text?.body?.substring(0,30)||msg.type}"`);
