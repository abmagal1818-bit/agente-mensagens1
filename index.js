  ...
  try {
    ...
    let msgsHtml = mensagens.map(m => {
      ...
      if (tipo !== 'client' && m.wamid) {
        ...
        const tituloCompleto = (m.status_entrega === 'failed' && m.motivo_erro) ? (s.titulo + ': ' + m.motivo_erro) : s.titulo;
        statusIcone = ' <span title="' + tituloCompleto.replace(/"/g, '') + '" style="color:' + s.cor + '">' + s.icone + '</span>';
      }
