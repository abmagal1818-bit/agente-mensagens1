...
app.get("/painel/chat/:tel", async (req, res) => {
  ...
  try {
    ...
    const html = '<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' + formatado + '</title>' +
      ...
      '<label style="font-size:11px;color:#81c784">Modelo do veículo para enviar as fotos:</label>' +
      '<input type="text" name="modelo" placeholder="ex: ASX, Cobalt, Argo..." required style="background:#1a1a1a;border:1px solid #2a2a2a;border-radius:6px;color:#fff;padding:8px;font-size:13px" value="' + (referralCliente ? String(referralCliente).match(/\b([A-Z][a-z]+(?:\s[A-Z][a-z]+)?)\b/)?.[1] || '' : '') + '">' +
...
app.post("/painel/enviar-fotos", async (req, res) => {
  const { tel, modelo } = req.body;
  if (!tel || !modelo) return res.redirect("/painel/lista");
  const modeloLower = modelo.toLowerCase();
  const veiculo = estoqueAtual.find(v => limparTexto(v.modelo || "").toLowerCase().includes(modeloLower));
  if (!veiculo || !veiculo.fotos?.length) {
