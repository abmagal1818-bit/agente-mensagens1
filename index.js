...
function exigirToken(req, res, next) {
  ...
  // Usa comparação timing-safe para evitar timing attacks — comparação
  // direta (===) vaza informação sobre o token correto pelo tempo de resposta.
  function tokenValido(t) {
    if (!t || t.length !== PAINEL_TOKEN.length) return false;
    try {
      return crypto.timingSafeEqual(Buffer.from(t), Buffer.from(PAINEL_TOKEN));
    } catch { return false; }
  }

  if (tokenValido(tokenQuery) || tokenValido(tokenCookie)) {
    if (tokenValido(tokenQuery)) {
      // Flag Secure garante que o cookie só é enviado em HTTPS
  ...
  try {
    ...
    for (const [telefone, ultima] of Object.entries(ultimaMensagemCliente)) {
      if (agora - ultima > 24 * 60 * 60 * 1000) {
        ...
        await supabase.from("clientes").upsert({ telefone, ultima_mensagem_cliente: new Date(ultima).toISOString() }, { onConflict: "telefone" }).catch(() => {});
        delete ultimaMensagemCliente[telefone];
      }
    }
    // Recupera do banco clientes que sumiram mas o servidor reiniciou antes de detectar
    // (resolve o problema de ultimaMensagemCliente ser perdida a cada reinício)
    ...
    if (clientesSumidosBanco?.length) {
      for (const cli of clientesSumidosBanco) {
        ...
        await supabase.from("clientes").update({ ultima_mensagem_cliente: null }).eq("telefone", cli.telefone).catch(() => {});
      }
    }
...
// ─────────────────────────────────────────────
// EXPURGO DE DADOS — LGPD (#7)
// ─────────────────────────────────────────────
// A LGPD exige que dados pessoais sejam mantidos apenas pelo tempo necessário.
// CPF e data de nascimento em simulações com mais de 1 ano são anonimizados
// (não deletados — o histórico de negociação ainda tem valor para análise,
// mas os dados identificadores são removidos). Roda uma vez por dia.
async function expurgarDadosAntigos() {
  try {
    const umAnoAtras = new Date();
    umAnoAtras.setFullYear(umAnoAtras.getFullYear() - 1);
    const { data, error } = await supabase
      .from("simulacoes_credito")
      .update({ cpf: "[removido - LGPD]", nascimento: "[removido - LGPD]" })
      .lt("criado_em", umAnoAtras.toISOString())
      .not("cpf", "eq", "[removido - LGPD]");
    if (!error) console.log("[LGPD] ✅ Expurgo executado — dados pessoais de simulações antigas anonimizados");
    else console.error("[LGPD] Erro no expurgo:", error.message);
  } catch (e) { console.error("[LGPD] Exceção no expurgo:", e.message); }
}
setInterval(expurgarDadosAntigos, 24 * 60 * 60 * 1000); // roda uma vez por dia
expurgarDadosAntigos(); // roda na inicialização também
...
async function processarMensagem(from, text, tentativasAnteriores = 0) {
  ...
  supabase.from("clientes").upsert({ telefone: from, ultima_mensagem_cliente: new Date().toISOString() }, { onConflict: "telefone" }).catch(() => {});
  ...
  if (!jaEstaNoHistorico) {
    // Remove CPF e data de nascimento do texto antes de passar para a IA —
    // esses dados não precisam ir para a Anthropic para que a conversa
    // funcione corretamente. A Sarah já sabe que recebeu o CPF pelo fluxo
    // de coleta (estado.cpf), então substituir por marcador não prejudica
    // o contexto. O CPF completo vai apenas pro WhatsApp do consultor e
    // pro Supabase (via notificarDadosCredito), não para a IA.
    const textParaIA = text
      .replace(/\b\d{3}\.?\d{3}\.?\d{3}-?\d{2}\b/g, "[CPF fornecido]")
      .replace(/\b\d{2}\/\d{2}\/\d{4}\b/g, "[data de nascimento fornecida]");
    conversas[from].push({ role: "user", content: textParaIA });
  }
  ...
  const clientePedindoFotos = ehTextoNormal && !jaEnviouFotos && clienteEstaPedindoFotosDoEstoque(text, conversas[from]);
  if (clientePedindoFotos) {
    ...
    if (veiculo?.fotos?.length > 0) {
      ...
      if (enviouComSucesso) {
        conversas[from].push({ role: "user", content: `[Sistema: fotos enviadas do ${limparTexto(veiculo.modelo)}. Confirme o envio e pergunte o que achou. Se o cliente também perguntou sobre financiamento ou troca, responda essas perguntas na mesma mensagem.]` });
...
// Rate limiting simples por IP para o webhook — evita que atacantes
// bombardeiem o endpoint consumindo créditos da Anthropic.
...
app.post("/webhook", async (req, res) => {
  ...
  // Validação HMAC da assinatura da Meta — garante que a requisição
  // veio realmente da Meta e não foi forjada por terceiros.
  // Requer APP_SECRET configurado no Render (disponível em Meta for Developers
  // → seu app → Configurações → Básico → "Segredo do aplicativo").
  const appSecret = process.env.META_APP_SECRET;
  if (appSecret) {
    const signature = req.headers["x-hub-signature-256"];
    if (!signature) {
      console.error("[Webhook] Requisição sem assinatura X-Hub-Signature-256 rejeitada");
  ...
  if (body.object === "whatsapp_business_account") {
    ...
    const from = msg.from;
    // Quando o cliente chega via anúncio Click-to-WhatsApp do Instagram/
    ...
    try {
      if (msg.type === "text") {
        ...
        if (!text) return;
        // Mascara CPF e data de nascimento no log do Render para não expor
        // dados pessoais em logs públicos/compartilhados. O CPF continua
        // chegando completo no WhatsApp do consultor (necessidade operacional).
        const textLog = text
          .replace(/\b\d{3}\.?\d{3}\.?\d{3}-?\d{2}\b/g, "***.***.***-**")
          .replace(/\b\d{2}\/\d{2}\/\d{4}\b/g, "**/**/****");
        console.log(`Texto de ${from}: ${textLog}`);
...
app.get("/painel/chat/:tel", async (req, res) => {
  ...
  try {
    ...
    let msgsHtml = mensagens.map(m => {
      ...
      const bg = (tipo === 'client' || tipo === 'client_foto') ? '#1e1e1e' : tipo === 'sara' ? '#1a3a1a' : tipo === 'sistema' ? '#1a1a2e' : '#2a1a00';
      const cor = (tipo === 'client' || tipo === 'client_foto') ? '#ddd' : tipo === 'sara' ? '#b8e6b8' : tipo === 'sistema' ? '#555' : '#f0c060';
      const label = (tipo === 'client' || tipo === 'client_foto') ? '👤 Cliente' : tipo === 'sara' ? '🤖 Sarah' : tipo === 'sistema' ? '⚙️ Sistema' : '⚡ Você';

      // Fotos recebidas de clientes — exibe a imagem diretamente no chat
      // em vez de só o texto "[Cliente enviou foto]". A URL da Meta expira
      // em ~5 minutos, então para fotos antigas mostra o ícone + análise.
      let conteudo = '';
      if (tipo === 'client_foto') {
        try {
