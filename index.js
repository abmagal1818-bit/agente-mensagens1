...
app.post("/webhook", async (req, res) => {
  ...
  if (webhookRateLimit[ip].count > WEBHOOK_LIMITE_POR_MINUTO) {
    console.error(`[Webhook] Rate limit excedido: ${ip}`);
    return res.sendStatus(429);
  }

  // Validação HMAC — usa rawBody capturado pelo express.json verify
  const appSecret = process.env.META_APP_SECRET;
  if (appSecret) {
    const signature = req.headers["x-hub-signature-256"];
    console.log(`[Webhook] Assinatura recebida: ${signature ? signature.substring(0, 20) + "..." : "AUSENTE"}`);
    if (!signature) {
      console.error("[Webhook] Sem assinatura — rejeitado");
      return res.sendStatus(403);
    }
    const rawBody = req.rawBody || Buffer.from(JSON.stringify(req.body));
    console.log(`[Webhook] rawBody disponível: ${!!req.rawBody}, tamanho: ${rawBody.length}`);
    const expectedSig = "sha256=" + crypto.createHmac("sha256", appSecret).update(rawBody).digest("hex");
    console.log(`[Webhook] Assinatura esperada: ${expectedSig.substring(0, 20)}...`);
    try {
    ...
    console.log("[Webhook] META_APP_SECRET não configurado — pulando validação HMAC");
  }
