...
app.get("/painel", async (req, res) => {
  ...
  try {
    ...
    const html = '<!DOCTYPE html><html lang="pt-BR"><head>' +
      ...
      '  fetch("/painel/iniciar-contato", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ telefone: tel, veiculo }) })' +
      '    .then(r => r.json())' +
...
app.post("/painel/iniciar-contato", async (req, res) => {
  ...
  try {
    ...
    if (enviou) {
      ...
      res.status(500).json({ erro: "Falha ao enviar template" });
    }
  } catch(e) {
