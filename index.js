...
✅ *SIMULACAO ${telefone} [resultado]* — ex: SIMULACAO ${telefone} Aprovado BV, parcela R$ 1.250 em 48x`;
  try {
    ...
    console.log(`[Crédito] ✅ Consultor notificado sobre dados de ${telefone} | CPF: ${mascararCPF(dados.cpf)}`);
...
async function salvarSimulacaoCredito(telefone, dados) {
  try {
    await supabase.from("simulacoes_credito").insert({
      telefone, nome: dados.nome, cpf: mascararCPF(dados.cpf), nascimento: dados.nascimento,
...
async function buscarEstoqueSupabase() {
  try {
    console.log("[Supabase] Buscando estoque...");
    const { data, error } = await supabase
      .from("veiculos")
      .select("id,marca,modelo,versao,spec_completa,ano_modelo,km,preco,descricao,opcionais,fotos,foto_capa,combustivel,cambio,portas,cor,status")
      .eq("publicado", true)
    ...
    const veiculos = (data || []).map(v => ({
      id: v.id,
      modelo: `${v.marca} ${v.modelo}${v.versao ? " " + v.versao : ""}`,
      ano: String(v.ano_modelo || ""),
      km: v.km || 0,
      preco: v.preco || 0,
      descricao: [
        v.spec_completa || "",
        v.combustivel ? `Combustivel: ${v.combustivel}` : "",
        v.cambio ? `Cambio: ${v.cambio}` : "",
        v.cor ? `Cor: ${v.cor}` : "",
        v.opcionais && v.opcionais.length ? `Opcionais: ${v.opcionais.join(", ")}` : "",
        v.descricao || ""
      ].filter(Boolean).join(" | "),
      fotos: v.foto_capa ? [v.foto_capa, ...(v.fotos || []).filter(f => f !== v.foto_capa)] : (v.fotos || []),
      atualizadoEm: new Date().toISOString()
    }));

    console.log(`[Supabase] ✅ ${veiculos.length} veiculos carregados`);
    return veiculos;
  } catch (e) {
    console.error("[Supabase] Erro ao buscar estoque:", e.message);
    return [];
  }
}
...
async function buscarEstoqueInstagram() {
  ...
  const veiculosSupabase = await buscarEstoqueSupabase();
  if (veiculosSupabase.length > 0) return veiculosSupabase;

  // Fallback Instagram se Supabase vazio
  console.log("[Instagram] Fallback: buscando posts...");
  try {
    const veiculos = [];
...
function clienteEstaPedindoFotosDoEstoque(texto, historicoConversa) {
  ...
  const ePedido = [
    ...
    "gostaria de foto", "gostaria de ver", "gostaria ver",
    "fotos do", "fotos da", "fotos de", "foto do", "foto da", "foto de",
    "ver o carro", "ver a foto", "ver foto", "fotografias",
    "queria ver", "queria as foto", "queria foto"
  ];
...
async function processarMensagem(from, text, tentativasAnteriores = 0) {
  ...
  const veiculoAtualCtx = encontrarVeiculoNoContexto(text, conversas[from], estoqueAtual);
  const nomeVeiculoAtual = veiculoAtualCtx ? limparTexto(veiculoAtualCtx.modelo || "").toLowerCase().split(" ").slice(0,2).join(" ") : "";
  const jaEnviouFotos = nomeVeiculoAtual.length > 2 && conversas[from].slice(-4).map(m => m.content || "").join(" ").toLowerCase().includes(`fotos enviadas do ${nomeVeiculoAtual}`);
